...
{
  // _dpa trace
  /* error_log("tFPDF: tFPDF construct"); 
  if (defined('FPDF_FONTPATH')) {
	error_log("tFPDF: FPDF_FONTPATH is defined: ".FPDF_FONTPATH);
  }
  if (defined('_SYSTEM_TTFONTS')) {
	error_log("tFPDF: _SYSTEM_TTFONTS is defined: "._SYSTEM_TTFONTS);
    } */ 
	...
	elseif(is_dir(dirname(__FILE__).'/font'))
		...
		$this->fontpath = '';
    // error_log("tFPDF::tFPDF: ". $this->fontpath); 
	// Core fonts
...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
      // error_log("tfpdf: file: ".$file);
		if (defined('_SYSTEM_TTFONTS') && file_exists(_SYSTEM_TTFONTS.$file )) { 
          // error_log("tfpdf: file exists "._SYSTEM_TTFONTS.$file);
          $ttffilename = _SYSTEM_TTFONTS.$file ; 
        } else { 
          // error_log("tfpdf: fontpath ".$this->_getfontpath());
          $ttffilename = $this->_getfontpath().$file ; // _dpa nounifontprefix
        }
		$unifilename = $this->_getfontpath().strtolower(substr($file ,0,(strpos($file ,'.'))));  // _dpa nounifontprefix
		$name = '';
		$originalsize = 0;
        // error_log("tfpdf: ttffilename is ".$ttffilename); 
		...
		if (!isset($type) ||  !isset($name) || $originalsize != $ttfstat['size']) {
			...
			$s.='$fontkey=\''.$fontkey."';\n";
			$s.="?>";
			if (is_writable(dirname($this->_getfontpath().'x'))) { // _dpa nounifontprefix
