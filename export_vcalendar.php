...
function export_ical($user_login, $date_debut, $date_fin, $mysql_link, $DEBUG=FALSE)
{
	...
	else
	{
		...
		//$sql_periodes="SELECT p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_commentaire, p_type  " .
				//"FROM conges_periode, conges_artt WHERE p_login='".$_SESSION['userlogin']."' AND (p_etat=('ok') OR (p_etat='demande')) AND ((p_date_deb>='$good_date_debut' AND  p_date_deb<='$good_date_fin') AND (p_date_fin>='$good_date_debut' OR p_date_fin<='$good_date_fin'))";
	      $sql_periodes="SELECT p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_commentaire, p_type " .
				  "FROM conges_periode  WHERE p_login='".$_SESSION['userlogin']."' AND (p_etat=('ok') OR (p_etat='demande')) AND ((p_date_deb>='$good_date_debut' AND  p_date_deb<='$good_date_fin') AND (p_date_fin>='$good_date_debut' AND p_date_fin<='$good_date_fin'))";
                                                                                         //ajout de OR (p_etat='demande') et remplacement du OR final de la ligne du dessus par AND par SA
		...
		if($num_periodes=mysql_num_rows($res_periodes)!=0)
		{
			while ($result_periodes = mysql_fetch_array($res_periodes))
			{
				...
				if($sql_demi_jour_deb=="am")
					$DTSTART=$tab_date_deb[0].$tab_date_deb[1].$tab_date_deb[2]."T070000";   // .....suppression du "Z"final par SA le 04/03/10
				else
					$DTSTART=$tab_date_deb[0].$tab_date_deb[1].$tab_date_deb[2]."T120000";   // .....suppression du "Z"final par SA le 04/03/10

				if($sql_demi_jour_fin=="am")
					$DTEND=$tab_date_fin[0].$tab_date_fin[1].$tab_date_fin[2]."T120000";   // .....  suppression du "Z"final par SA le 04/03/10
				else
					$DTEND=$tab_date_fin[0].$tab_date_fin[1].$tab_date_fin[2]."T180000";   // .....  suppression du "Z"final par SA le 04/03/10

					echo "BEGIN:VEVENT\r\n" .
						"DTSTAMP:$DTSTAMP\r\n" .
						"ORGANIZER;CN=".$_SESSION['userlogin'].":MAILTO:".$tab_infos_user['email']."\r\n" .
						"CREATED:$DTSTAMP\r\n" .
						"UID:php_conges:$DTSTART\r\n" .//ajout de "php_conges:$DTSTART" par SA le 04/03/10
						"SEQUENCE:0\r\n" .
...
function export_vcal($user_login, $date_debut, $date_fin, $mysql_link, $DEBUG=FALSE)
{
	...
	else
	{
		...
		// on prend toutes les periodes de conges qui chevauchent la periode donnée par les dates demandées
		//$sql_periodes="SELECT p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_commentaire, p_type  " .
				"FROM conges_periode WHERE p_login='".$_SESSION['userlogin']."' AND (p_etat=('ok') OR (p_etat='demande')) AND (p_date_deb>='$good_date_debut' AND  p_date_deb<='$good_date_fin') OR (p_date_fin>='$good_date_debut' AND p_date_fin<='$good_date_fin')";
		  $sql_periodes="SELECT p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_commentaire, p_type  " .
						  "FROM conges_periode WHERE p_login='".$_SESSION['userlogin']."' AND (p_etat=('ok') OR (p_etat='demande')) AND ((p_date_deb>='$good_date_debut' AND  p_date_deb<='$good_date_fin') AND (p_date_fin>='$good_date_debut' AND p_date_fin<='$good_date_fin'))";
                                                                                         //suppression du OR ligne du dessus le 10/03/10 par SA
