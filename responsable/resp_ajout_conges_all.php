...
*/	
			// on insert l'ajout de conges dans la table periode _ac3 
            if ($tab_commentaire_saisie[$user_name] == "") { 
	...
	foreach($tab_new_nb_conges_all as $id_conges => $nb_jours)
	{
		if($nb_jours!=0)
		{
			...
			while($resultat1 = mysqli_fetch_array($ReqLog1)) 
			{
                ...
                // _ac3: conserve le comment specifique s'il est rempli 
                if ($tab_new_comment_all[$id_conges] == "") { 
                  $commentaire = $_SESSION['lang']['resp_ajout_conges_comment_periode_all'];
                } else {
                  $commentaire = $tab_new_comment_all[$id_conges] ;
                }
				// ajout conges
