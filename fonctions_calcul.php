...
function compter($user, $date_debut, $date_fin, $opt_debut, $opt_fin, $revised_period=-1,&$comment, $mysql_link, $DEBUG=FALSE)
{
  error_log("compter: ".$user."-". $date_debut."-". $date_fin."-". $opt_debut."-". $opt_fin);
  $nbj = compter_ori($user, $date_debut, $date_fin, $opt_debut, $opt_fin, $revised_period,$comment, $mysql_link, $DEBUG);
  error_log("compter: ".$nbj." ". $comment);
  return $nbj;   
}
...
function compter_ori($user, $date_debut, $date_fin, $opt_debut, $opt_fin, $revised_period=-1,&$comment, $mysql_link, $DEBUG=FALSE)
{
	...
	if( ($date_debut!=0) && ($date_fin!=0) )
	{
		...
		/************************************************************/
		// 2 : on verifie que le conges demandé ne chevauche pas une periode deja posée
		if(verif_periode_chevauche_periode_user($date_debut, $date_fin, $user, $tab_periode_calcul,$revised_period, $comment, $mysql_link, $DEBUG) == TRUE)
			return 0;
...
function verif_periode_chevauche_periode_groupe($date_debut, $date_fin, $tab_periode_calcul, $groupe_id, $mysql_link, $DEBUG=FALSE)
{
	...
	foreach($tab_users as $current_login)
	{
		...
		$comment="";
		if(verif_periode_chevauche_periode_user($date_debut, $date_fin, $current_login, $tab_periode_calcul,-1,$comment, $mysql_link, $DEBUG)==TRUE)
			return TRUE;
	}
}
...
// retourne TRUE si chevauchement et FALSE sinon !
// ac3 $revised_period numero de period dont on reevalue la quotite de jour 
//     cette periode sera exclue du calcul de recouvrement 
function verif_periode_chevauche_periode_user($date_debut, $date_fin, $user, $tab_periode_calcul, 
                                              $revised_period=-1, &$comment, $mysql_link, $DEBUG=FALSE)
{
		/************************************************************/
		// 2 : on verifie que le conges demandé ne chevauche pas une periode deja posée
		...
		while($current_day!=$date_limite)
		{
			...
			$user_periode_sql = "SELECT p_num, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_etat
							FROM conges_periode
